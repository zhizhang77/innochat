...
class ModelsStore {
	...
	modelSupportsVision(modelId: string): boolean {
		if (this.isModelExternal(modelId)) return true;
	...
	private isModelExternal(modelIdOrName: string): boolean {
		return !!(this.findModelById(modelIdOrName)?.external || this.findModelByName(modelIdOrName)?.external);
	}

	isModelLoaded(modelId: string): boolean {
		// external models are always "loaded"
		if (this.isModelExternal(modelId)) return true;
	...
	private async runFetch(): Promise<void> {
		...
		try {
			...
			this.models = [...this.models, ...this.buildExternalModelOptions()];
		} catch (error) {
			this.models = [];
	...
	private buildExternalModelOptions(): ModelOption[] {
		return settingsStore.externalModels.map((em) => ({
			id: `external:${em.id}`,
			name: em.name,
			model: em.modelId,
			external: true,
	...
	async loadModel(modelId: string): Promise<void> {
		...
		if (this.isModelExternal(modelId)) return;
	...
	async unloadModel(modelId: string): Promise<void> {
		// external models don't need unloading
		if (this.isModelExternal(modelId)) return;
		if (!this.isModelLoaded(modelId)) return;
		if (this.modelLoadingStates.get(modelId)) return;
	...
	async ensureModelLoaded(modelId: string): Promise<void> {
		// external models are always "ready"
		if (this.isModelExternal(modelId)) return;
		if (this.isModelLoaded(modelId)) return;
	...
	/**
	 * Sync external models from settings store without re-fetching local models.
	 */
	syncExternalModels(): void {
		this.models = [
			...this.models.filter((m) => !m.external),
			...this.buildExternalModelOptions()
		];
	}
}
