...
import type { ExternalModel, SettingsExportType } from '$lib/types';
import { setMode } from 'mode-watcher';
import {
	CONFIG_LOCALSTORAGE_KEY,
	EXTERNAL_MODELS_LOCALSTORAGE_KEY,
...
class SettingsStore {
	...
	externalModels = $state<ExternalModel[]>([]);
	...
	private loadExternalModels() {
		if (!browser) return;
		try {
			const raw = localStorage.getItem(EXTERNAL_MODELS_LOCALSTORAGE_KEY);
			if (raw) {
				this.externalModels = JSON.parse(raw);
			} else {
				this.seedDefaultExternalModel();
			}
		} catch (error) {
			console.warn('Failed to load external models from localStorage:', error);
			this.externalModels = [];
		}
	}

	private seedDefaultExternalModel() {
		try {
			const envDefault = import.meta.env.VITE_PUBLIC_DEFAULT_EXTERNAL_MODEL;
			if (!envDefault) return;
			const parsed = JSON.parse(envDefault);
			if (parsed && parsed.name && parsed.baseUrl && parsed.modelId) {
				this.externalModels = [{
				...
				this.saveExternalModels();
			}
		} catch {
			// env var not set or invalid JSON — silently skip
		}
	}

	private saveExternalModels() {
		if (!browser) return;
		try {
			localStorage.setItem(EXTERNAL_MODELS_LOCALSTORAGE_KEY, JSON.stringify(this.externalModels));
		} catch (error) {
			console.error('Failed to save external models to localStorage:', error);
		}
	}

	addExternalModel(model: Omit<ExternalModel, 'id' | 'createdAt'>): ExternalModel {
		const newModel: ExternalModel = {
			...model,
			id: crypto.randomUUID(),
			createdAt: Date.now()
		};
		this.externalModels = [...this.externalModels, newModel];
		this.saveExternalModels();
		this.notifyExternalModelsChanged();
		return newModel;
	}

	updateExternalModel(id: string, updates: Partial<Omit<ExternalModel, 'id' | 'createdAt'>>) {
		this.externalModels = this.externalModels.map((m) =>
			m.id === id ? { ...m, ...updates } : m
		);
		this.saveExternalModels();
		this.notifyExternalModelsChanged();
	...
	removeExternalModel(id: string) {
		this.externalModels = this.externalModels.filter((m) => m.id !== id);
		this.saveExternalModels();
		this.notifyExternalModelsChanged();
	}

	private notifyExternalModelsChanged() {
		import('$lib/stores/models.svelte').then(({ modelsStore }) => {
			modelsStore.syncExternalModels();
		});
	}
